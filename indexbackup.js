...
const queue = new Map();
const youtube = new YouTube('AIzaSyAaUBbsWgimby_ecoMBEroli226Y-7LdRU');
...
client.on('message', async (msg) => {
  ...
  if(msg.channel.name == undefined) {
    ...
    if(command === 'play') {
      ...
      try {
        var video = await youtube.getVideo(args[1] ? args[1].replace(/<(._)/g, '$1') : '')

      } catch {
        try {
          var videos = await youtube.searchVideos(args.slice(0).join(' '), 1)
          var video = await youtube.getVideoByID(videos[0].id)
        } catch (error) {
          if(error.code === 403) return msg.channel.send('I\'ve exceeded my Quota limit :sob: , no music until tommorow')
    ...
    if(command === 'skip') {
      ...
      const skipembed = new Discord.MessageEmbed()
        ...
        .setFooter(`Nebula Bot`, 'https://cdn.discordapp.com/avatars/777741089641463811/22528105555c847c2dd1a10194ff37f5.webp?size=256')
      if(serverQueue.songs[1]) skipembed.setDescription(`**Skipped**: ${serverQueue.songs[0].title} \n \n **Now Playing**: ${serverQueue.songs[1].title}`)
    ...
    if(command === 'queue') {
      ...
      var lines = serverQueue.songs.map(song => `${song.title}`).join(`\n`).split('\n')
      lines.splice(0,1)
